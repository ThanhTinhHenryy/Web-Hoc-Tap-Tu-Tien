...
const Confetti = ({ isActive }) => {
  ...
  useEffect(() => {
    if (isActive) {
      ...
      const animations = ['animate-fall-slow', 'animate-fall-medium', 'animate-fall-fast'];
    ...
    <div className="fixed inset-0 pointer-events-none z-50 overflow-hidden">
      {confetti.map((piece) => {
        ...
        if (piece.shape === 'circle') {
          shapeElement = <div className="rounded-full" style={{ width: piece.size, height: piece.size, backgroundColor: piece.color }}></div>;
        } else if (piece.shape === 'square') {
          shapeElement = <div style={{ width: piece.size, height: piece.size, backgroundColor: piece.color }}></div>;
